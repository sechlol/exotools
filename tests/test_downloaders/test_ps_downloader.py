...
class TestPlanetarySystemsDownloader:
    @pytest.fixture
    def tap_qtable_data(self, planetary_systems_test_data) -> QTable:
        """Create a modified copy of the test data to simulate raw data from TAP service"""
        # Get test data and make a copy to modify
        in_table = planetary_systems_test_data[0].copy()
    ...
    @pytest.fixture
    def descriptions(self, planetary_systems_test_data) -> dict[str, str]:
        """Extract descriptions from the test data header"""
        header = planetary_systems_test_data[1]
    ...
    def test_download_smoketest(
        ...
        with patch("exotools.downloaders.ps_downloader.ExoService") as mock_exo_service_factory:
            ...
            downloader = PlanetarySystemsDownloader()
